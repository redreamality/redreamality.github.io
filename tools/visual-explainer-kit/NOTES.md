...
`pnpm visual-kit:e2e` 先运行 manifest/schema、重复步骤 ID 和缺失 demo 的负向校验，再覆盖三语加载、语言切换、图内/aria 本地化、暂停、重置、reduced-motion、可选 overview、可变步骤数和第二主题。

## 正式集成边界

- 站点采用“专区目录 + 独立 HTML 体验”的混合模型；工具是可选 authoring path，不进入访客运行时。
- Astro visual manifest 管理路由、SEO、语言可用性和专区卡片；本工具只负责生成自包含体验页。
- `typhoon/v2` 的 14 个 demo 若要完全模板化，仍需逐个把可见及 aria 文案提取到 locale copy；首发版本保留经过验证的中文独立 HTML。
