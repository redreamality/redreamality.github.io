...
* **规划（Planning）**：
  ...
  * **Single-Agent タスク Decomposition**：在单个智能体中，任务分解涉及将大任务分解为一系列可管理的小任务。

* **记忆/上下文管理（メモリ Management）**：

  * **Short-term メモリ**：在对话或交互期间使用的即时、短暂的记忆。

  * **Long-term メモリ**：存储历史对话和响应的记忆。

  * **External データ ストレージ**：如RAG（Lewis et al., 2020），用于补充信息源。

  * **Episodic メモリ**：涉及多智能体系统中一系列交互的记忆。

  * **Consensus メモリ**：在多智能体系统中，作为共享信息的统一来源。
